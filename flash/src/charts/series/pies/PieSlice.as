...
//
		...
		public function get_colour(): Number {
			if (this.colour is SolidColor)
				return SolidColor(this.colour).color;
			else if (this.colour is GradientBase && GradientBase(this.colour).entries.length > 0) {
				var ge : GradientEntry = GradientBase(this.colour).entries[0];
				return ge.color;
			} else
				return Utils.get_colour('#fff');
		}
	}
}
