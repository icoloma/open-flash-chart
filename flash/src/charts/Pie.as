package charts {
	...
	public class Pie extends Base
	{
		...
		public function Pie( json:Object )
		{
			...
			if (json['gradients'] == null) {
				
				object_helper.merge_2( json, this.style );			

				if (json['gradientFill'] == null) {  
					for each( var colour:String in this.style.colours ) {
						this.colours.push( new SolidColor(string.Utils.get_colour( colour ) ));
					}
				} else { // deprecated way of gradientFill
					for each( colour in this.style.colours ) {
						var rg: RadialGradient = new RadialGradient();
						var c: Number = string.Utils.get_colour(colour);
						rg.entries = [
							new GradientEntry(c, -1, 1),
							new GradientEntry(c, -1, .5)
						];
						this.colours.push(rg);
					}
				}			
				
			} else {
			
				for each( var gradientEntries:Array in json.gradients ) {
					rg = new RadialGradient();
					var entries: Array = [];
					for each( var gradientEntry : Object in gradientEntries) {
						entries.push(new GradientEntry(
							string.Utils.get_colour(gradientEntry.colour), 
							parseFloat(gradientEntry.ratio), 
							parseFloat(gradientEntry.alpha)
							));
					}
					rg.entries = entries;
					this.colours.push(rg);
...
//			this.Elements= new Array();
		...
		private function add_slice( index:Number, start:Number, angle:Number, value:Object, tip:String, colour:IFill ): PieSliceContainer {
